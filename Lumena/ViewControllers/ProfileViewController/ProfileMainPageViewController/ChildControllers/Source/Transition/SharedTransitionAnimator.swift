...
class SharedTransitionAnimator: NSObject {
    ...
    static var cellFrame: CGRect = CGRect(x: 0, y: 0, width: ((UIScreen.main.bounds.width - 4.5)/3), height: ((UIScreen.main.bounds.width - 4.5)/3))
}
...
//        }
        ...
        UIView.animate(duration: config.duration, curve: config.curve, options: config.animationOptions) { [config] in
