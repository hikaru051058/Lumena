...
struct ImageExtractorAsset: Identifiable {
    ...
    static func create(from url: URL) async throws -> ImageExtractorAsset? {
        let (data, response) = try await downloadAsset(from: url)
        
        guard let data = data else {
            print("Error downloading asset: No data received")
            return nil
        }
        
        // Determine if the file is an image or a video
        if let mimeType = response.mimeType, mimeType.starts(with: "image") {
            return try await handleImageData(data)
    ...
    private static func downloadAsset(from url: URL) async throws -> (Data?, URLResponse) {
        let (data, response) = try await URLSession.shared.data(from: url)
        return (data, response)
    }
    
    private static func handleImageData(_ data: Data) async throws -> ImageExtractorAsset? {
        guard let image = UIImage(data: data) else {
            print("Error creating image from data")
            return nil
        }
        
        // Create a PHAsset placeholder (if necessary, replace this with actual asset creation)
        let placeholderAsset = PHAsset()
        
        return ImageExtractorAsset(asset: placeholderAsset, image: image, isVideo: false)
    }
    
    private static func handleVideoURL(_ url: URL) -> ImageExtractorAsset? {
        // Create a PHAsset placeholder (if necessary, replace this with actual asset creation)
        let placeholderAsset = PHAsset()
        
        return ImageExtractorAsset(asset: placeholderAsset, videoURL: url, isVideo: true)
    }
}

actor ImageDownloader {
    var downloadedAssets: [ImageExtractorAsset] = []
    
    func addAsset(_ asset: ImageExtractorAsset) {
        downloadedAssets.append(asset)
    }
    
    func getAssets() -> [ImageExtractorAsset] {
