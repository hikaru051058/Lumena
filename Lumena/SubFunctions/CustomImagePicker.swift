...
enum Crop: Equatable{
    ...
    func size()->CGSize{
        ...
        case .rectangle:
            return .init(width: 600, height: 1000)
        case .square:
...
extension View{
    ...
    func cropImagePicker(option: Crop, show: Binding<Bool>, croppedImage: Binding<UIImage?>) -> some View {
...
class ImagePickerViewModel: ObservableObject {
    ...
    @Published var selectedCropType: Crop = .circle
}
...
fileprivate struct CustomImagePicker<Content: View>: View {
    ...
    init(option: Crop, show: Binding<Bool>, croppedImage: Binding<UIImage?>, @ViewBuilder content: @escaping () -> Content) {
    ...
    var body: some View {
        ...
        content
            ...
            .onChange(of: photosItem) { newValue in
                ...
                Task {
                    do {
                        guard let firstItem = newValue else {
                            // If no items are selected, clear the selectedImage
                            selectedImage = nil
                            return
                        }
                        ...
                        if let data = try await firstItem.loadTransferable(type: Data.self) {
                            if let uiImage = UIImage(data: data) {
                                imagePickerViewModel.selectedImage = uiImage
                                imagePickerViewModel.selectedCropType = option
                                imagePickerViewModel.showCropView.toggle()
                            }
                        }
                    } catch {
                        print(error)
            ...
            .fullScreenCover(isPresented: $imagePickerViewModel.showCropView) {
                
            } content: {
                CropView(crop: imagePickerViewModel.selectedCropType, image: imagePickerViewModel.selectedImage) { croppedImage, status in
                    if let croppedImage {
                        self.croppedImage = croppedImage
                        imagePickerViewModel.selectedImage = nil
                    }
                }
            }
    }
