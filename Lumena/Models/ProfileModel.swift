...
class ProfileManager: ObservableObject {
    ...
    func getProfile(withID id: String) async throws -> ProfileSettings {
        if let existingProfile = profiles[id] {
            ...
            let profile = try await getProfileAPI(with: id) ?? ProfileSettings(id: id)
    ...
    func getProfileQueue(withID id: String) async throws {
        if !profiles.values.contains(where: { $0.identityID == id }) {
            ...
            let profile = ProfileSettings(id: id)
    ...
    func updateFollowingStatus(fromUserID: String, toUserID: String, follow: Bool) {
        Task {
            do {
                // Fetch current relationship status if it exists or default to .none
...
extension ProfileSettings {
    ...
    func fetchUserImages() async throws {
        do {
            let result = try await GraphQL.shared.fetchUserProfileImages(userID: self.identityID)
            DispatchQueue.main.async {
                self.profileImage = Lumena.profileImage(url: result.profileUrl)
                self.backgroundImage = Lumena.profileImage(url: result.backgroundUrl)
            }
        } catch {
            print(error)
        }
